...
def _merge_feat_imp(imp_df1, imp_df2, feature_name, top_n, color_dict):
    ...
    imp_df2['pos'] = 'right'
    if top_n:
        both_imp = imp_df1.head(top_n).merge(
            imp_df2.head(top_n), on=feature_name, how='outer')
    else:
        both_imp = imp_df1.merge(imp_df2, on=feature_name, how='outer')

    both_imp['bar_color'] = both_imp.apply(
        lambda x: _rank2color(x, color_dict), axis=1)
    both_imp['bar_mark'] = both_imp.apply(lambda x: _get_mark(x), axis=1)

    return both_imp


def feature_drift_graph(
        feat_imp1,
        feat_imp2,
        feature_name,
        imp_name,
    ...
    bar_left_data = both_imp[['feat_name',
                              'relative_imp_x',
                              'pos_x',
                              'bar_color',
                              'bar_mark']].dropna().sort_values('relative_imp_x',
                                                                ascending=False)
    bar_left_data.columns = [col.replace('_x', '')
                             for col in bar_left_data.columns.values]

    bar_right_data = both_imp[['feat_name',
                               'relative_imp_y',
                               'pos_y',
                               'bar_color',
                               'bar_mark']].dropna().sort_values('relative_imp_y',
                                                                 ascending=False)
    bar_right_data.columns = [
        col.replace(
            '_y',
            '') for col in bar_right_data.columns.values]

    line_data = both_imp[['feat_name', 'bar_color', 'feat_rank_x', 'feat_rank_y']].dropna()[
        ['feat_name', 'bar_color']]
    ...
    temp = open(
        os.path.join(
            ...
            'src/feature_drift_template.html'
            )
    ).read()
    ...
    with open(output_path, 'wb') as fh:
        fh.write(
            template.render(
                {
                    'bar_left_data': bar_left_data.to_dict('records'),
