...
def _get_tree_info(
    ...
    # check target names
    if not isinstance(target_names, list) or len(
            target_names) != tree_model.tree_.n_classes:
        raise ValueError(
            "target_names should be a list of length %d." %
            (tree_model.tree_.n_classes))
...
def _parse_tree(node_id, parent, pos, tree_info):
    ...
    if parent == 'null':
        ...
        if pos == 'left':
            if feature in binary_features:
                ...
                node['name'] = feature + " <= " + \
                    str(int(tree_model.tree_.threshold[parent]))
            else:
                node['name'] = feature + " <= " + \
                    str(round(tree_model.tree_.threshold[parent], 3))
        else:
            if feature in binary_features:
                node['name'] = feature + ': 1'
            elif feature in int_features:
    ...
    if tree_model.tree_.children_left[node_id] != - \
            1 or tree_model.tree_.children_right[node_id] != -1:
        node['children'] = []
            ...
            for k in node['features'].keys():
                ...
                if k in tree_info['binary_features']:
                    ...
                    if feat[0] == -sys.maxsize:
                        ...
                        rule = str(round(feat[0], 3)) + ' < ' + \
                            k + ' <= ' + str(round(feat[1], 3))
                rules.append(rule)
...
def generate_simple_tree(
    ...
    with open('simple_tree_output/simple_tree_%s.html' % (tree_title), 'wb') as fh:
        ...
        print(
            'The output is in simple_tree_output/simple_tree_%s.html. Enjoy!' %
            (tree_title))


def generate_sankey_tree(
        tree_title,
        tree_model,
        X,
        target_names,
    ...
    with open('sankey_tree_output/sankey_tree_%s.html' % (tree_title), 'wb') as fh:
        render_result = {
            ...
            'max_samples': np.max(
                tree_info['tree_model'].tree_.n_node_samples),
            'min_samples': np.min(
                tree_info['tree_model'].tree_.n_node_samples),
        }
        fh.write(template.render(render_result))
        print(
            'The output is in sankey_tree_output/sankey_tree_%s.html. Enjoy!' %
            (tree_title))
